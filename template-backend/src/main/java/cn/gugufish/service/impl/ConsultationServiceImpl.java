...
public class ConsultationServiceImpl implements ConsultationService {
    ...
    public ConsultationRecordVO recordDetail(int accountId, int id) {
        ...
        return record.asViewObject(ConsultationRecordVO.class, vo -> {
            ...
            vo.setArchiveSummary(buildArchiveSummary(
                    record,
                    messageSummary,
                    assignment,
                    doctorHandle,
    ...
    private ConsultationArchiveSummaryVO buildArchiveSummary(ConsultationRecord record,
        ...
        String doctorName = resolveArchiveDoctorName(doctorHandle, assignment, triageResult);
        ...
        List<String> nextActions = buildArchiveNextActions(record, messageSummary, doctorHandle, doctorConclusion, latestFollowUp, serviceFeedback);

        ConsultationArchiveSummaryVO summary = new ConsultationArchiveSummaryVO();
        summary.setStageText(resolveArchiveStageText(record, doctorHandle, doctorConclusion, latestFollowUp));
        summary.setOverview(buildArchiveOverview(record));
        summary.setTriageSummary(buildArchiveTriageSummary(record, triageResult, riskFlags));
        summary.setDoctorSummary(buildArchiveDoctorSummary(doctorHandle, doctorName));
        summary.setArchiveConclusion(buildArchiveConclusion(doctorHandle, doctorConclusion));
        summary.setFollowUpSummary(buildArchiveFollowUpSummary(doctorConclusion, latestFollowUp, doctorFollowUps));
        summary.setServiceSummary(buildArchiveServiceSummary(record, doctorHandle, serviceFeedback));
        summary.setLatestMessageSummary(buildArchiveLatestMessageSummary(messageSummary));
        summary.setPatientActionHint(resolveArchiveActionHint(nextActions, record, doctorHandle, doctorConclusion));
        summary.setDoctorName(doctorName);
        summary.setLastUpdateTime(resolveArchiveLastUpdateTime(record, messageSummary, doctorHandle, doctorConclusion, latestFollowUp, serviceFeedback, triageResult));
        ...
        summary.setPlainText(buildArchivePlainText(record, summary));
        return summary;
    }

    private String resolveArchiveStageText(ConsultationRecord record,
                                           ConsultationDoctorHandleVO doctorHandle,
                                           ConsultationDoctorConclusionVO doctorConclusion,
                                           ConsultationDoctorFollowUpVO latestFollowUp) {
        if (latestFollowUp != null && java.util.Objects.equals(latestFollowUp.getNeedRevisit(), 0)) {
            return "本轮问诊与随访已完成";
        }
        if (doctorConclusion != null) {
            return java.util.Objects.equals(doctorConclusion.getNeedFollowUp(), 1)
                    ? "医生已完成结论，进入随访观察"
                    : "医生已完成本轮结论归档";
        }
    ...
    private String buildArchiveOverview(ConsultationRecord record) {
        List<String> segments = new ArrayList<>();
        String title = firstText(record.getTitle(), record.getCategoryName());
        if (title != null) segments.add("问诊主题：" + title);
        if (trimToNull(record.getPatientName()) != null) segments.add("就诊人：" + record.getPatientName());
        ...
        return segments.isEmpty() ? "已提交本次问诊资料，等待系统继续处理。" : String.join("；", segments);
    }

    private String buildArchiveTriageSummary(ConsultationRecord record,
                                             TriageResultVO triageResult,
        ...
        if (departmentName != null) segments.add("推荐科室：" + departmentName);
        String triageReason = firstText(
                triageResult == null ? null : triageResult.getReasonText(),
                record.getTriageSuggestion()
        );
        if (triageReason != null) segments.add("导诊建议：" + abbreviate(triageReason, 160));
        if (riskFlags != null && !riskFlags.isEmpty()) {
            segments.add("重点风险：" + abbreviate(String.join("、", riskFlags), 80));
        }
        return segments.isEmpty() ? "系统尚未生成完整的导诊结果。" : String.join("；", segments);
    }

    private String buildArchiveDoctorSummary(ConsultationDoctorHandleVO doctorHandle,
                                             String doctorName) {
        if (doctorHandle == null) {
            return doctorName == null
                    ? "医生尚未开始正式处理，当前仍在等待接诊。"
                    : "当前由" + doctorName + "跟进，尚未形成完整处理记录。";
        }
        List<String> segments = new ArrayList<>();
        ...
        return segments.isEmpty() ? "医生已接诊，正在整理处理意见。" : String.join("；", segments);
    }

    private String buildArchiveConclusion(ConsultationDoctorHandleVO doctorHandle,
                                          ConsultationDoctorConclusionVO doctorConclusion) {
        if (doctorConclusion == null) {
            if (doctorHandle != null && trimToNull(doctorHandle.getSummary()) != null) {
                return "医生已完成初步处理，结构化结论尚未补充完整。";
            }
            return "当前尚未形成最终归档结论。";
        }
        List<String> segments = new ArrayList<>();
        ...
        if (trimToNull(doctorConclusion.getPatientInstruction()) != null) {
            segments.add("患者指导：" + abbreviate(doctorConclusion.getPatientInstruction(), 160));
        }
    ...
    private String buildArchiveFollowUpSummary(ConsultationDoctorConclusionVO doctorConclusion,
                                               ConsultationDoctorFollowUpVO latestFollowUp,
                                               List<ConsultationDoctorFollowUpVO> doctorFollowUps) {
        if (latestFollowUp == null) {
            if (doctorConclusion != null && java.util.Objects.equals(doctorConclusion.getNeedFollowUp(), 1)) {
                String followUpText = buildConclusionFollowUpText(doctorConclusion);
                return followUpText == null
                        ? "医生建议继续随访，当前尚未记录新的随访结果。"
                        : "医生建议" + followUpText + "，当前尚未记录新的随访结果。";
            }
            return "当前暂无随访记录，也未要求继续随访。";
        }
        List<String> segments = new ArrayList<>();
        ...
        if (java.util.Objects.equals(latestFollowUp.getNeedRevisit(), 0)) {
            segments.add("本轮随访已完成");
        }
        return String.join("；", segments);
    }

    private String buildArchiveServiceSummary(ConsultationRecord record,
                                              ConsultationDoctorHandleVO doctorHandle,
                                              ConsultationServiceFeedbackVO serviceFeedback) {
    ...
    private String buildArchiveLatestMessageSummary(ConsultationMessageSummaryVO messageSummary) {
        ...
        String latestSender = latestMessageSenderText(messageSummary);
        ...
        if (defaultInt(messageSummary.getUnreadCount()) > 0) {
            segments.add("未读医生回复：" + messageSummary.getUnreadCount() + " 条");
        }
        return String.join("；", segments);
    }

    private List<String> buildArchiveNextActions(ConsultationRecord record,
                                                 ConsultationMessageSummaryVO messageSummary,
                                                 ConsultationDoctorHandleVO doctorHandle,
                                                 ConsultationDoctorConclusionVO doctorConclusion,
                                                 ConsultationDoctorFollowUpVO latestFollowUp,
        ...
        if (messageSummary != null && defaultInt(messageSummary.getUnreadCount()) > 0) {
            addArchiveAction(actions, "查看医生最新回复，并按建议继续处理。");
        }
        ...
        if ("emergency".equalsIgnoreCase(disposition)) {
            addArchiveAction(actions, "如症状持续加重，请立即前往线下急诊。");
        } else if ("offline_visit".equalsIgnoreCase(disposition)) {
            addArchiveAction(actions, "医生建议尽快线下就医或完善检查。");
        } else if ("online_followup".equalsIgnoreCase(disposition)) {
            addArchiveAction(actions, "如症状仍有变化，可继续在线补充恢复情况。");
        } else if ("observe".equalsIgnoreCase(disposition)) {
            addArchiveAction(actions, "当前以居家观察为主，如症状加重请及时复诊。");
        }

        if (doctorConclusion != null && java.util.Objects.equals(doctorConclusion.getNeedFollowUp(), 1)) {
            if (latestFollowUp != null && java.util.Objects.equals(latestFollowUp.getNeedRevisit(), 0)) {
                addArchiveAction(actions, "本轮随访已完成，如后续再次不适可重新发起问诊。");
            } else if (latestFollowUp != null && latestFollowUp.getNextFollowUpDate() != null) {
                addArchiveAction(actions, "请在" + formatDateOnly(latestFollowUp.getNextFollowUpDate()) + "前后反馈恢复情况，便于医生继续随访。");
            } else if (doctorConclusion.getFollowUpWithinDays() != null) {
                addArchiveAction(actions, "建议在" + doctorConclusion.getFollowUpWithinDays() + "天内补充恢复情况，便于继续随访。");
            } else {
                addArchiveAction(actions, "当前仍处于随访观察期，可继续补充恢复情况。");
            }
        }

        String patientInstruction = trimToNull(doctorConclusion == null ? null : doctorConclusion.getPatientInstruction());
        if (patientInstruction != null) {
            addArchiveAction(actions, abbreviate("留意医生提示：" + patientInstruction, 140));
        }

        String medicalAdvice = trimToNull(doctorHandle == null ? null : doctorHandle.getMedicalAdvice());
        if (medicalAdvice != null) {
            addArchiveAction(actions, abbreviate("处理建议：" + medicalAdvice, 140));
        }

        if (doctorHandle == null) {
            addArchiveAction(actions, "等待医生接诊期间，可继续补充症状变化、检查结果或图片资料。");
        }

        boolean completed = "completed".equalsIgnoreCase(trimToNull(record.getStatus()))
                || (doctorHandle != null && "completed".equalsIgnoreCase(trimToNull(doctorHandle.getStatus())));
        if (completed && serviceFeedback == null) {
            addArchiveAction(actions, "本轮处理结束后，可补充服务评价帮助完善问诊体验。");
        }
        if (actions.isEmpty()) {
            addArchiveAction(actions, "留意平台消息通知，如症状变化可继续补充说明。");
        }
        return actions.stream().limit(5).toList();
    }

    private String resolveArchiveActionHint(List<String> nextActions,
                                            ConsultationRecord record,
                                            ConsultationDoctorHandleVO doctorHandle,
                                            ConsultationDoctorConclusionVO doctorConclusion) {
        if (nextActions != null && !nextActions.isEmpty()) return nextActions.get(0);
        if (doctorConclusion != null && java.util.Objects.equals(doctorConclusion.getNeedFollowUp(), 1)) {
            return "当前仍处于随访观察期，建议继续反馈恢复情况。";
        }
        if (doctorHandle != null) return "医生正在持续跟进本次问诊，请留意后续消息。";
        if ("triaged".equalsIgnoreCase(trimToNull(record.getStatus()))) return "导诊已完成，正在等待医生接诊。";
        return "当前暂无额外操作要求，请留意后续通知。";
    }

    private Date resolveArchiveLastUpdateTime(ConsultationRecord record,
                                              ConsultationMessageSummaryVO messageSummary,
                                              ConsultationDoctorHandleVO doctorHandle,
                                              ConsultationDoctorConclusionVO doctorConclusion,
                                              ConsultationDoctorFollowUpVO latestFollowUp,
    ...
    private String resolveArchiveDoctorName(ConsultationDoctorHandleVO doctorHandle,
                                            ConsultationDoctorAssignment assignment,
                                            TriageResultVO triageResult) {
        return firstText(
    ...
    private ConsultationDoctorFollowUpVO latestFollowUp(List<ConsultationDoctorFollowUpVO> doctorFollowUps) {
        return doctorFollowUps == null || doctorFollowUps.isEmpty() ? null : doctorFollowUps.get(0);
    }

    private String buildArchivePlainText(ConsultationRecord record,
                                         ConsultationArchiveSummaryVO summary) {
        StringBuilder builder = new StringBuilder();
        appendArchivePlainText(builder, "问诊编号", record.getConsultationNo());
        appendArchivePlainText(builder, "当前阶段", summary.getStageText());
        appendArchivePlainText(builder, "跟进医生", summary.getDoctorName());
        appendArchivePlainText(builder, "最近更新", formatDateTime(summary.getLastUpdateTime()));
        appendArchivePlainText(builder, "问诊概览", summary.getOverview());
        appendArchivePlainText(builder, "导诊结论", summary.getTriageSummary());
        appendArchivePlainText(builder, "医生处理", summary.getDoctorSummary());
        appendArchivePlainText(builder, "归档结论", summary.getArchiveConclusion());
        appendArchivePlainText(builder, "随访进展", summary.getFollowUpSummary());
        appendArchivePlainText(builder, "服务评价", summary.getServiceSummary());
        appendArchivePlainText(builder, "最近沟通", summary.getLatestMessageSummary());
        if (summary.getRiskFlags() != null && !summary.getRiskFlags().isEmpty()) {
            appendArchivePlainText(builder, "风险提示", String.join("、", summary.getRiskFlags()));
        }
        if (summary.getConclusionTags() != null && !summary.getConclusionTags().isEmpty()) {
            appendArchivePlainText(builder, "结论标签", String.join("、", summary.getConclusionTags()));
        }
        if (summary.getNextActions() != null && !summary.getNextActions().isEmpty()) {
            builder.append("后续建议：").append('\n');
    ...
    private void appendArchivePlainText(StringBuilder builder,
                                        String label,
                                        String value) {
    ...
    private void addArchiveAction(List<String> actions,
                                  String action) {
    ...
    private Date maxDate(Date left,
                         Date right) {
    ...
    private String latestMessageSenderText(ConsultationMessageSummaryVO messageSummary) {
        if (messageSummary == null) return null;
        String senderName = trimToNull(messageSummary.getLatestSenderName());
        String senderType = trimToNull(messageSummary.getLatestSenderType());
        String roleText = "doctor".equalsIgnoreCase(senderType)
                ? "医生"
                : "user".equalsIgnoreCase(senderType)
                ? "患者"
                : "系统";
        return senderName == null ? roleText : senderName + "（" + roleText + "）";
    }

    private String doctorHandleStatusText(String status) {
        return "completed".equalsIgnoreCase(trimToNull(status))
                ? "已完成本轮处理"
                : "正在处理中";
    }

    private String conditionLevelText(String value) {
        return switch (trimToNull(value) == null ? "" : value.toLowerCase()) {
            case "low" -> "轻度";
            case "medium" -> "中度";
            case "high" -> "较高风险";
            case "critical" -> "危急";
    ...
    private String dispositionText(String value) {
        return switch (trimToNull(value) == null ? "" : value.toLowerCase()) {
            case "observe" -> "继续观察";
    ...
    private String buildConclusionFollowUpText(ConsultationDoctorConclusionVO doctorConclusion) {
        if (doctorConclusion == null) return null;
        if (java.util.Objects.equals(doctorConclusion.getNeedFollowUp(), 1)) {
            return doctorConclusion.getFollowUpWithinDays() == null
                    ? "需要继续随访"
                    : doctorConclusion.getFollowUpWithinDays() + " 天内随访";
        }
        if (java.util.Objects.equals(doctorConclusion.getNeedFollowUp(), 0)) {
            return "暂不需要继续随访";
        }
        return null;
    }

    private String followUpTypeText(String value) {
        return switch (trimToNull(value) == null ? "" : value.toLowerCase()) {
            case "platform" -> "平台随访";
            case "phone" -> "电话随访";
            case "offline" -> "线下随访";
    ...
    private String patientStatusText(String value) {
        return switch (trimToNull(value) == null ? "" : value.toLowerCase()) {
            case "improved" -> "明显好转";
            case "stable" -> "基本稳定";
            case "worsened" -> "出现加重";
    ...
    private String serviceResolvedText(Integer value) {
        if (value == null) return "待确认";
        return value == 1 ? "问题已解决" : "问题仍需继续跟进";
    }
